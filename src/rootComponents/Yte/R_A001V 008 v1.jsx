...
// Component tính toán codeFrame và imgFrame
const FrameCalculator = ({ items, volume = 2, onDataReady }) => {
  ...
  // ✅ FIX: Xử lý trường hợp không có audio files
  useEffect(() => {
    if (validItemsCount === 0) {
      console.log("⚠️ No audio files to load, processing immediately");
      ...
      console.log("📊 codeFrame:", tempCodeFrame);
      console.log("📸 imgFrame:", mergedImgFrame);
    }
  }, [validItemsCount, items, handle, fps, onDataReady]);

  useEffect(() => {
    if (validItemsCount > 0 && loadingCount >= validItemsCount && isLoading) {
      console.log("✅ All audio durations loaded!");
      let accumulatedFrames = 0;
      const tempCodeFrame = [];
      const tempImgFrame = [];
      ...
      items.forEach((e, i) => {
        ...
        // ✅ SPREAD toàn bộ properties từ item
        tempCodeFrame.push({
          ...e, // Giữ lại tất cả thuộc tính: text, code, textView, typingtext, typingSound, img, timePlus, etc.
          startFrame: accumulatedFrames,
          endFrame: accumulatedFrames + duration,
          duration: duration,
        });
...
export const VideoTemplate = ({ item, duration }) => {
  ...
  // State để lưu codeFrame và imgFrame từ FrameCalculator
  const [codeFrame, setCodeFrame] = useState([]);
  const [imgFrame, setImgFrame] = useState([]);
  const [isDataReady, setIsDataReady] = useState(false);

  // Callback nhận data từ FrameCalculator
    ...
    >
      ...
      <Sequence from={0}>
        <VideoPlay
          ...
          videoSource="ytemp4"
          zoom={1}
          zIndex={0} // ✅ Tăng zIndex để video nổi lên trên
          loop={true}
          sound={false}
        />
        {generateVideoSegments(duration, [
          ...
          <VideoSmallPlay
            ...
            videoSource="ytemp4"
            zoom={1}
            zIndex={10} // ✅ Tăng zIndex để video nổi lên trên
            loop={false}
            sound={false}
        ...
        >
          ...
          >
            ...
            <div className="col-10">
              ...
              {isDataReady && (
                <TextView codeFrame={codeFrame} textEnd={"Bsi PHAM THE HIEN"} />
              )}
            </div>{" "}
          </div>{" "}
        </div>
...
function generateVideoSegments(
  ...
  while (startFrameBegin < duration) {
    ...
    // Cập nhật startFrameBegin cho lần tiếp theo
    startFrameBegin = endFrame;

    // Chuyển sang timeSet tiếp theo
    timeSetIndex++;

    // Dừng nếu đã đạt duration
